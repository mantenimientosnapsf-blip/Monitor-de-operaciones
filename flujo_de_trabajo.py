...
def mostrar_graficos():
    ...
    # Botón de regreso que limpia la pantalla reescribiendo el estado
    ...
    def crear_anillo(df, titulo):
        try:
            ...
            if resumen.empty: return st.write(f"Sin datos: {titulo}")
            ...
            fig = px.pie(resumen, values='Total_Hechas', names='motivo', hole=0.55, color='motivo', color_discrete_map=colores_snap)
            fig.update_traces(textinfo='none', hoverinfo='label+percent', rotation=120, direction='clockwise', marker=dict(line=dict(color='#FFFFFF', width=2)))
            fig.update_layout(
                title=dict(text=titulo, x=0.26, y=0.88, xanchor='center', font=dict(size=20, color='black', family="Arial Black")),
                showlegend=True, legend=dict(orientation="v", yanchor="middle", y=0.5, xanchor="left", x=1.05, font=dict(color='black', size=14)),
                margin=dict(t=80, b=20, l=10, r=10), height=450,
                annotations=[dict(text=f"TOTAL<br><b>{total_general}</b>", x=0.5, y=0.5, font_size=18, showarrow=False, font_color='black', font_family="Arial Black")]
            )
            fig.for_each_trace(lambda t: t.update(labels=[f"{n} (<b>{ (v/total_general*100):.1f}%</b>)" for n, v in zip(resumen['motivo'], resumen['Total_Hechas'])]))
    ...
    def sector_inferior_estadisticas(df_final):
        try:
            ...
            try: df_pend = pd.read_sql("SELECT * FROM PENDIENTES", conn)
            except: df_pend = pd.DataFrame()
            conn.close()
            ...
            with col_izq:
                ...
                if not df_pend.empty:
                    ...
                    fig_p.update_layout(xaxis_title=None, yaxis_title="", height=450, margin=dict(t=20, b=20, l=10, r=10), xaxis=dict(tickangle=-90, tickfont=dict(color='#000000', size=18, family="Arial Black")), yaxis=dict(dtick=1, range=[0, resumen_p['Cantidad'].max() * 1.3]), plot_bgcolor='rgba(0,0,0,0)')
            ...
            with col_der:
                ...
                if not resumen_mes.empty:
                    ...
                    fig_m.update_layout(xaxis_title=None, yaxis_title=None, height=450, margin=dict(t=20, b=100, l=10, r=10), xaxis=dict(tickangle=-90, tickfont=dict(color='#000000', size=16, family="Arial Black")), yaxis=dict(dtick=50 if max_m > 100 else 10, range=[0, max_m * 1.3]), plot_bgcolor='rgba(0,0,0,0)')
        ...
        except Exception as e: st.error(f"Error en sector inferior: {e}")
    ...
    if os.path.exists(db_path):
        ...
        else: titulo_semana = "ESTA SEMANA"
