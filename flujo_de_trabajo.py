...
def mostrar_graficos():
    ...
    def crear_anillo(df, titulo, col_render):
        try:
            df = df.copy()
            ...
            if resumen.empty:
                with col_render: st.write(f"Sin datos: {titulo}")
                return
    ...
    # --- ENCAPSULAMIENTO DINÁMICO CONTRA DUPLICACIÓN ---
    # Al meter un key que cambia según la sección, forzamos la recarga limpia de los gráficos
    with st.container(key=st.session_state["seccion_activa"]):
        if os.path.exists(db_path):
            ...
            crear_anillo(df_año, "AÑO ACTUAL", c1)
            crear_anillo(df_mes, "MES ACTUAL", c2)
            crear_anillo(df_semana, titulo_semana, c3)
