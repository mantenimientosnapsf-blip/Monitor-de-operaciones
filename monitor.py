...
with col1:
    ...
    if not nov_df.empty:
        for _, row in nov_df.iterrows():
            ...
            info_texto = f"{rango_fecha} | {row['hi']} a {row['hf']} hs" if es_horario else f"Fecha: {rango_fecha}"
            st.markdown(f"""<div class="{clase}"><b>{row['p']}</b><br>{row['t'].upper()}<br><small>{info_texto}</small></div>""", unsafe_allow_html=True)
...
with col3:
    ...
    WHERE p.lug NOT IN ('TALLER SANTA FE', 'VIAJE A BAHÍA BLANCA', 'VIAJE A NECOCHEA', 'VIAJE A CÓRDOBA', 'BUNGE TANCACHA')
    """)
    if not int_df.empty:
